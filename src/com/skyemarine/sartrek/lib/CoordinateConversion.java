	...
	*/
	...
	private class LatLon2UTM {
		public String convertLatLonToUTM(double latitude, double longitude) {
			...
			// I added this formatting...
	        String easting = Integer.toString((int)_easting);
	        while (easting.length() < 7)
	            easting = "0" + easting;
	        String northing = Integer.toString((int)_northing);
	        while (northing.length() < 7)
	            northing = "0" + northing;

	        UTM = longZone + latZone + " " + easting + " " + northing;
		...
		protected void setVariables(double latitude, double longitude) {
			...
			K2 = nu * SIN(latitude) * COS(latitude) * POW(sin1, 2) * k0
					* (100000000) / 2;
			K3 = ((POW(sin1, 4) * nu * SIN(latitude) * MathUtilities.pow(COS(latitude),
					3)) / 24)
	...
	private class UTM2LatLon {
		...
		public double[] convertUTMToLatLong(String UTM) {
			...
			// Split UTM string
			String[] utm = new String[4];
			int start = 0;
			int space = UTM.indexOf(" ");
			utm[0] = UTM.substring(start, space);
			start = space;
			space = UTM.indexOf(" ", space);
			utm[1] = UTM.substring(start, space);
			start = space;
			space = UTM.indexOf(" ", space);
			utm[2] = UTM.substring(start, space);
			utm[3] = UTM.substring(space);
